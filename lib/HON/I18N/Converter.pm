...
#La fonction shift prend un tableau en argument; elle supprime son premier ŽlŽment
#(les autres sont alors dŽcalŽs) et renvoie cet ŽlŽment.
		my ($self) = shift;

		#DŽclaration tableau vide
		my @line = ();

		for my $worksheet ( $self->workbook->worksheets() ) {
			my ( $col_min, $col_max ) = $worksheet->col_range();

#RŽcupŽration des cellules de la premire ligne (ligne correspondant ˆ la langue)
			for my $col ( $col_min .. $col_max ) {

				#Valeur de la cellule
				my $cell = $worksheet->get_cell( 0, $col );

				#Va ˆ la prochaine cellule sauf si la cellule est vide
				next unless $cell;
...
=cut
	...
	sub p_write_JS_i18n {
		...
		foreach my $lang ( keys %{ $self->labels } ) {
			...
			#IntitulŽ de chaque section
			$content .= "\$.i18n.$lang = $json;\n";
		}

		#Dernire ligne du document jQuery
